...
def find_team_candidates(name):
    ...
    cache_key = f"{search_key}_{len(ALL_TEAMS)}"
    
    now = time.time()
    if cache_key in SEARCH_CACHE and now - SEARCH_CACHE[cache_key]['time'] < SEARCH_CACHE_TTL:
        return SEARCH_CACHE[cache_key]['data']
    
    candidates = []
    for tid, tname, tshort, tla, lid, lname in ALL_TEAMS:
        score = max(
        ...
        if score > 0.4:
            candidates.append((score, tname, tid, tla, lid, lname))
    
    candidates.sort(reverse=True)
    result = candidates[:5]
    SEARCH_CACHE[cache_key] = {'time': now, 'data': result}
    return result
